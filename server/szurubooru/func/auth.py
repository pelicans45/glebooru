...
REVERSED_RANK_MAP = util.flip(RANK_MAP)
...
def is_valid_token(user_token: Optional[model.UserToken]) -> bool:
    ...
    if user_token.expiration_time is not None:
        if user_token.expiration_time.tzinfo is not None:
            now = datetime.now(user_token.expiration_time.tzinfo)
        else:
            now = datetime.now(UTC).replace(tzinfo=None)
        if user_token.expiration_time < now:
            return False
    return True
...
def has_privilege(user: model.User, privilege_name: str) -> bool:
    ...
    return user.rank in ALL_RANKS[ALL_RANKS.index(minimal_rank) :]
