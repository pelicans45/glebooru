...
def test_refresh_metrics(tag_factory, metric_factory):
    ...
    assert config.all_metrics[0].tag.names[0].name == 'tag1'
    assert config.all_metrics[1].tag.names[0].name == 'tag2'
...
def test_filter_anonymous(
    ...
    metric1 = metric_factory(tag1)
    metric2 = metric_factory(tag2)
    t1_10 = post_metric_factory(metric=metric1, value=10)
    t1_30 = post_metric_factory(metric=metric1, value=30)
    t2_20 = post_metric_factory(metric=metric2, value=20.5)
