...
class PostMetricInputControl extends events.EventTarget {
    constructor(hostNode, ctx) {
        ...
        this._metricListNode = editAreaNode.querySelector(
            "ul.compact-unset-metrics"
        );
        this._separatorNode = editAreaNode.querySelector("hr.separator");
        this._postMetricListNode = editAreaNode.querySelector(
    ...
    refreshContent() {
        ...
        this._separatorNode.style.display = this._postMetricListNode.innerHTML
            ? "block"
            : "none";
    }
    ...
    _createPostMetricNode(pm) {
        ...
        node.querySelector("input[name=value]").addEventListener(
            "change",
            (e) => {
    ...
    _createPostMetricRangeNode(pmr) {
        ...
        node.querySelector("input[name=low]").addEventListener(
            "change",
            (e) => {
        ...
        node.querySelector("input[name=high]").addEventListener(
            "change",
            (e) => {
    ...
    createPostMetric(tag) {
        let postMetricRange = this._post.metricRanges.findByTagName(
            tag.names[0]
        );
        if (postMetricRange) {
    ...
    createPostMetricRange(tag) {
        ...
        this._post.metricRanges.add(
            PostMetricRange.create(this._post.id, tag)
        );
        this.refreshContent();
        this.dispatchEvent(new CustomEvent("change"));
    }
