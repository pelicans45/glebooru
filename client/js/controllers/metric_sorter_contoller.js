...
class MetricSorterController {
    constructor(ctx) {
        ...
        this._view.addEventListener("changeMetric", (e) =>
            this._evtChangeMetric(e)
        );
    ...
    startSortingRandomPost() {
        this._view.clearMessages();
        this._getRandomUnsortedPostId()
            .then((id) => {
                this._ctx.parameters.id = id;
                router.replace(
                    views.getMetricSorterUrl(id, this._ctx.parameters)
                );
    ...
    reloadMedianPost() {
        ...
        this._tryGetMedianPost(metricName, range)
            .then((medianResponse) => {
                if (medianResponse.post) {
                    ...
                    let exactValue =
                        (medianResponse.range.low +
                            medianResponse.range.high) /
                        2;
                    this._view.showSuccess(`Found exact value: ${exactValue}`);
                    this._setExactMetric(
                        this._unsortedPost,
                        metricName,
                        exactValue
                    );
    ...
    _evtSubmit(e) {
        let range = this._getOrCreateRange(
            this._unsortedPost,
            this._primaryMetricName
        );
        if (this._foundExactValue) {
            this._unsortedPost.metricRanges.remove(range);
        } else {
            let medianValue = this._sortedPost.metrics.findByTagName(
                this._primaryMetricName
            ).value;
