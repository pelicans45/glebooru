...
## Testing Guidelines
Pytest drives the backend suite (`server/szurubooru/tests/{api,func,model,...}`); mirror that layout when adding coverage. Name tests descriptively (`test_feature_behavior`) and rely on fixtures in `conftest.py` for setup. **Always run tests inside Docker** (never on the host), e.g. `docker compose -f docker-compose.dev.yml exec server pytest` or `docker compose -f docker-compose.dev.yml exec server pytest path/to/test_file.py::test_case`. No formal coverage gate exists, but contributions should add or update tests alongside feature and bugfix work.
